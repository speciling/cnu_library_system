...
def rental_books():
    ...
    if result == 1:
        # 2-1. 대출 가능한 책인지 판단!
        print(":: 대출하고싶은 도서의 ISBN을 입력하세요.")
        book_isbn = input(">> ISBN: ")
        count = book_yn(book_isbn, 'y')  # 도서 대출 가능 여부 확인
        ...
        if count == 1:  # 대출 가능!
            ...
            # 경고 메세지 출력 후 메인화면 전환
            print(f'#Waring: "{book_isbn}" 도서는 대출이 불가능합니다.')
            return
    else:
        # 경고 메세지 출력 후 메인화면 전환
...
# tbl_book 테이블의 도서 대출 가능 여부 변경(y->n or n->y)
...
def return_books():
    ...
    if result == 1:  # 회원인 경우
        ...
        if count == 1:  # 반납 가능
            ...
            print(f'#Waring: "{book_isbn}"은 반납이 불가능합니다.')
            return
